...
with tab1:
    ...
    def _build_corr_svg(corr_df: pd.DataFrame, size: int = 300) -> str:
        ...
        most_neg = min((float(corr_df.loc[r,c]) for r in labels for c in labels if r != c), default=0)
        most_pos = max((float(corr_df.loc[r,c]) for r in labels for c in labels if r != c), default=0)
        ...
        parts = [
            ...
            f'<desc id="{desc_id}">Heatmap of pairwise correlations for '
            f'{", ".join(labels)}. '
